...
def connect_vpn(server):
    ...
    # Pre-emptive cleanup of stale connection profile
...
def disconnect_vpn():
    if not is_active():
        # Even if not active, try to delete the profile in case it's lingering
        subprocess.run(["nmcli", "connection", "delete", CONNECTION_NAME], capture_output=True)
        return False, "No active VPN connection found to stop."
    ...
    if os.path.exists(PID_FILE):
        os.remove(PID_FILE)
    return True, "VPN disconnected and removed."
...
if __name__ == "__main__":
    ...
    filtered = []
    for s in servers:
        if proto_pref == "udp" and s['is_udp']: filtered.append(s)
        elif proto_pref == "tcp" and not s['is_udp']: filtered.append(s)
    ...
    try:
        ...
        if 0 <= idx < len(filtered):
            success, msg = connect_vpn(filtered[idx])
            print(msg)
        else:
            print("Invalid index.")
